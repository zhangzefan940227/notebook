...
**总结：**
- 常量 int 常量指针 p = 常量/非常量对象地址
- 常量 int 非常量指针 p = 常量/非常量对象地址
- 非常量 int 常量指针 p = 非常量对象地址（不可以是常量）
- 非常量 int 非常量指针 p = 非常量对象地址
简单的说
- 指针p指向的对象若是常量，那给p赋值的对象可以是常量，也可以是非常量。
- 指针p指向的对象若是非常量，那给p赋值的对象必须是非常量。
