...
Bundle savedInstanceState是用来保存数据，Activity要是因为各种原因被销毁了，但是Bundle savedInstanceState不会销毁，重启的时候会在onCreate这个生命周期的第一个方法中保存下来，系统回调的时候给你用。

==oncreate(Bundle saveInstanceState,PersisitanbleBundle persistentState)==

可以将数据更加持久化的保存在PersisitanbleBundle persistentState中，同时记得在清单文件中的Activity属性中添加android:persistableMode=“persistAcrossReboots”，这样之后方可消除空白问题，要是想要获取PersisitanbleBundle persistentState数据重写下面方法就行了。
...
* dispatchTouchEvent
  ...
  * return super：`View`没有拦截器，View类的super.dispatchTouchEvent()默认实现会调用View自己的onTouchEvent
